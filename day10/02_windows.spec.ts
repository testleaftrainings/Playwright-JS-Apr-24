...
test(`Handling windows`, async({context, page})=>{
    ...
    //allPages = [Amazon, Leafground, OneIndia, Flipkart]
    console.log(allPages);

    console.log(`No. of opened page: ${allPages.length}`);
...
/**
 * Sequential appraoch - one action after the other
 * Step 1: Create a promise in your code to tell that this will resolve into a new page
 *      const windowPromise = context.waitForEvent('page')
 * Step 2: Perform the user action which is supposed to open a new window
 *      page.locator("#Click")
 * Step 3: Now wait for the promise to get completed (for the promise you created in step 1)
 *      const window = await windowPromise;
 * Step 4: Switch to the new window for performing further actions
 *       window.bringToFront();
 * 
 * 1. Wait for the event
 * 2. user action - Click Open
 * 3. A new page is opened
 */
...
test(`Handling windows using sequential approach`, async({context, page})=>{
    ...
    const window = await windowPromise; //promise fulfilled
...
test.only(`Handling windows concurrently`, async({context,page})=>{
